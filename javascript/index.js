...
function createNewArr(arr1, arr2) {
  arr1.forEach(({ id, imgSrc, name, price }) => {
    const item = {
      id: id,
      imgSrc: imgSrc,
      name: name,
      price: price,
      disPrice: (price * discountRating).toFixed(2),
    };
    arr2.push(item);
  });
}
...
formSubscription.addEventListener("submit", (e) => {
  if (disountFuc()) {
    ...
    if (discountRating == 0.7 && discountRating1 === 0.7) {
      document.body.classList.add("discountActive");
    }
  }
  console.log(localStorage.setItem("discountRating", discountRating));
});
...
for (let i = 0; i < discountCategories.length; i++) {
  createNewArr(arrayCombination[i], discountCategories[i]);
}
